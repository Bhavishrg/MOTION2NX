...
#ifndef R_t_16
#define R_t_16             int8_t             // Ring data type for all the constructions
#endif
...
// FIXED DEFINITIONS
#define N_BITS_16          sizeof(R_t_16)*8                       // Number of bits in R_t_16

#define G_IN_LEN_16        CEIL(SEC_PARAM,8)                   // [SEC_PARAM/8] input bytes
#define OUT_LEN_16         CEIL(2*SEC_PARAM+2*N_BITS_16+2,8)      // output bytes
#define G_OUT_LEN_16       (CEIL(OUT_LEN_16,G_IN_LEN_16)*G_IN_LEN_16)   // output bytes of G

#define TO_R_t_16(ptr)     (*((R_t_16*)(ptr)))                    // Cast pointer to R_t_16
#define TO_BOOL_16(ptr)    ((bool)((*(uint8_t*)(ptr))&0x01))   // Cast pointer to bool

// Sizes of the various elements in the FSS key
#define S_LEN_16           G_IN_LEN_16                            // Size of the states s
#define V_LEN_16           sizeof(R_t_16)                         // Size of the masking values v
#define CW_LEN_16          (S_LEN_16 + sizeof(R_t_16) + 2)           // Size of the correction words
#define CW_CHAIN_LEN_16    ((CW_LEN_16*N_BITS_16)+V_LEN_16)             // Size of the correction word chain
#define KEY_LEN_16         (S_LEN_16 + CW_CHAIN_LEN_16 + V_LEN_16)      // Size of the FSS key

// Positions of the elements in the correction word chain, for each correction word j
#define S_CW_PTR_16(j)     (j*CW_LEN_16)                          // Position of state s_cw
#define V_CW_PTR_16(j)     (S_CW_PTR_16(j) + S_LEN_16)               // Position of value v_cw
#define T_CW_L_PTR_16(j)   (V_CW_PTR_16(j) + V_LEN_16)               // Position of bit t_cw_l
#define T_CW_R_PTR_16(j)   (T_CW_L_PTR_16(j) + 1)                 // Position of bit t_cw_r
#define LAST_CW_PTR_16     (CW_LEN_16*N_BITS_16)                     // Position of last correction word, v_cw_n+1

// Positions of left and right elements in the output of G 
#define S_L_PTR_16         0                                   // Position of state s_l in G output
#define S_R_PTR_16         (S_L_PTR_16 + S_LEN_16)                   // Position of state s_r in G output
#define V_L_PTR_16         (S_R_PTR_16 + S_LEN_16)                   // Position of value v_l in G output
#define V_R_PTR_16         (V_L_PTR_16 + V_LEN_16)                   // Position of value v_r in G output
#define T_L_PTR_16         (V_R_PTR_16 + V_LEN_16)                   // Position of bit t_l in G output
#define T_R_PTR_16         (T_L_PTR_16 + 1)                       // Position of bit t_r in G output

// Positions of the elements in the FSS key
#define S_PTR_16           0                                   // Position of state s
#define CW_CHAIN_PTR_16    (S_PTR_16 + S_LEN_16)                     // Position of correction word chain
#define Z_PTR_16           (CW_CHAIN_PTR_16 + CW_CHAIN_LEN_16)       // Position of value z
...
R_t_16 random_dtype_16();                                    // Non-deterministic seed
R_t_16 random_dtype_seeded_16(const uint8_t seed[SEED_LEN]);
void random_buffer_16(uint8_t buffer[], size_t buffer_len);   // Non-deterministic seed 
...
/// @param alpha input mask (should be uniformly random in R_t_16)
...
/// @param r_in     input mask (should be uniformly random in R_t_16)
/// @param r_out    output mask (should be uniformly random in R_t_16)
