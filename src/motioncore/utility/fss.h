...
#ifndef R_t
#define R_t             int64_t             // Ring data type for all the constructions
#endif
...
//--------------------------------  PRIVATE  ---------------------------------//
//----------------------------------------------------------------------------//
void xor_c(const uint8_t *a, const uint8_t *b, uint8_t *res, size_t s_len);
...
void DPF_gen(R_t alpha, uint8_t k0[KEY_LEN], uint8_t k1[KEY_LEN]);
void DPF_gen_seeded(R_t alpha, uint8_t k0[KEY_LEN], uint8_t k1[KEY_LEN], uint8_t s0[S_LEN], uint8_t s1[S_LEN]);
...
R_t DPF_eval(bool b, const uint8_t kb[KEY_LEN], R_t x_hat);
